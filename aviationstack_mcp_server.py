...
AVIATIONSTACK_API_KEY = os.getenv("AVIATIONSTACK_API_KEY") or os.getenv("AVIATION_STACK_API_KEY")
BASE_URL = "http://api.aviationstack.com/v1"

@mcp.tool()
async def search_flights(flight_iata: str = None, dep_iata: str = None, arr_iata: str = None):
    """
    Search live flight information using Aviationstack API.
    Provide flight_iata (e.g., 'AA100'), or departure/arrival airport IATA codes (e.g., dep_iata='JFK', arr_iata='LHR').
    """
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(f"{BASE_URL}/flights", params=params, timeout=10.0)
            data = response.json()
            if "error" in data:
                return f"API Error: {data['error']}"
            return str(data.get("data", []))[:2000]  # Return initial records
        except Exception as e:
            return f"Failed to fetch flight data: {str(e)}"
